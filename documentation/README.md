# ESP32 UNAPI Firmware Documentation
...
- Allow 1 active TCP connection only when using SSL, as it nearly sucks all RAM available for a program running on an ESP. Two connections could be possible depending upon the other end configuring less usage of buffers, but this is not reliable.
...
This firmware uses ESP32 Arduino IDE EEPROM functionality, that uses part of the flash memory that is not affected during firmware or file system updates to store a few bytes of data. 32 bytes of flash memory are reserved for this firmware, that are used as follow:
